...
class Store {
    ...
    changeRarity(rarId, name, newparamvalue) {
        this.raritys = this.raritys.map(x => (x.id === rarId ? { ...x, [name]: newparamvalue } : x));
    }

    changeRarityValue(rarId, name, newparamvalue) {
        this.raritys = this.raritys.map(x => (x.id === rarId ? { ...x, value_temporary: { ...x.value_temporary, [name]: newparamvalue } } : x));
    }
